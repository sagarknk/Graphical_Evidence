...
### Usage:
%%% Requires the same problem dimensions and settings as specified in GHS_graphical_evidence.m
%%% Reads the n x q data matrix stored after data generation in GHS_graphical_evidence.m
...
### Usage:
%%% Requires the same problem dimensions and settings as specified in GHS_graphical_evidence.m
%%% Reads the q x q sample covariance matrix stored after data generation in GHS_graphical_evidence.m
%%% Function calls are made to "prior_sampling_for_Neal_and_skilling.m" to propose $\Omega$. 
%%% log-marginal is estimated for 25 times in parallel, using the parallel-for "parfor".
%%% Mean and sd of the log-marginal estimates are printed at the end of computation. 
